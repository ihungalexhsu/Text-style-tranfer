...
class PickleDataset(Dataset):
    def __init__(self, pos_pickle_path, neg_pickle_path=None, config=None, sort=True):
        with open(pos_pickle_path, 'rb') as f:
            pos_data = pickle.load(f)
        if neg_pickle_path is not None:
            ...
            for k,v in neg_data.items():
                self.data_dict[k+len(pos_data)]=v
        else:
            self.data_dict=pos_data
    ...
    def __getitem__(self, index):
        ...
        align = self.data_dict[utt_id]['align']
        return token_ids, label, align
