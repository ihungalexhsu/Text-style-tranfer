...
class Style_transfer_proposed(object):
    def __init__(self, config, alpha=1, beta=1, gamma=100, delta=10, load_model=False):
        self.config = config
        print(self.config)
    ...
    def _get_decoder_pred(self, want_styles, content_vector, 
    ...
    def _get_m1_loss(self, data, domain_discriminator, tf_rate):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        xs, ys, ys_in, ys_out, ilens, styles = to_gpu(data, bos, eos, pad)
    ...
    def _log_m1(self, epoch, train_steps, total_steps, s_loss, l_recon, 
                l_adv_disencS, l_adv_disencC, l_mimic, l_adv_domain_discri, log_steps):
        print(f'epoch: {epoch}, [{train_steps + 1}/{total_steps}], '
    ...
    def _train_m1_onetime(self, data, total_adv_disencS, total_adv_disencC,
                          total_sloss, total_mimic, total_recon, total_adv_domain,
                          epoch, train_steps, total_steps, domain_discriminator, tf_rate):
        s_loss, l_adv_disencS, l_adv_disencC, l_mimic, l_recon, l_adv_domain_discri=\
        ...
        for i in range(len(self.train_pos_loader)):
            ...
            (train_steps, total_adv_posdiscri, total_recon, total_sloss, 
             total_adv_disencS, total_adv_disencC, total_mimic)=\
                self._train_m1_onetime(data, total_adv_disencS, total_adv_disencC,
                                       total_sloss, total_mimic, total_recon, 
                                       total_adv_negdiscri, epoch, train_steps, 
                                       total_steps, self.neg_domain_discri, tf_rate)
            try:
            ...
            (train_steps, total_adv_negdiscri, total_recon, total_sloss, 
             total_adv_disencS, total_adv_disencC, total_mimic)=\
                self._train_m1_onetime(data, total_adv_disencS, total_adv_disencC,
                                       total_sloss, total_mimic, total_recon, 
                                       total_adv_posdiscri, epoch, train_steps, 
                                       total_steps, self.pos_domain_discri, tf_rate)
