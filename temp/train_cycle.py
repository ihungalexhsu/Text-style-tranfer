...
class Cycle_model(object):
    ...
    def _get_loss(self, data, tf_rate, epoch):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        xs, ys, ys_in, ys_out, ilens, styles, aligns = to_gpu(data, bos, eos, pad)
        if epoch <= 4:
            mask_prob = 0.9-(0.9-0.2)*((epoch-0.0)/(4.0-0.0))
        else:
            mask_prob = 0.2
        masked = True if np.random.random_sample() <= mask_prob else False
        if masked:
            xs=xs.masked_fill_(aligns, pad)
        else:
            xs, ilens, ys, ys_in, ys_out, styles =\
                denoise(xs, ilens, ys, ys_in, ys_out, styles, self.vocab['<PAD>'])
        enc_output, enc_lens = self.encoder(xs, ilens)
        _, recon_log_probs, predictions, attns = self.decoder(enc_output, enc_lens, styles, aligns, (ys_in, ys_out), tf_rate)
        # Reconstruction loss
    ...
    def _get_cycle_loss(self, data, tf_rate):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        # calculate loss at first
        if self.prediction is not None:
            cycle_ilens = get_prediction_length(self.prediction, eos=self.vocab['<EOS>'])
            enc_output, enc_lens = self.encoder(self.prediction, cycle_ilens, need_sort=True)
            _, cycle_log_probs, predictions, attns =\
                self.decoder(enc_output, enc_lens, self.styles, self.aligns,
                             (self.ys_in, self.ys_out), tf_rate)
            cycle_loss = -torch.mean(cycle_log_probs)
        else:
            cycle_loss = 0
        # create new data  
        xs, ys, ys_in, ys_out, ilens, styles, aligns = to_gpu(data, bos, eos, pad)
        self.styles = copy.deepcopy(styles)
        self.aligns = copy.deepcopy(aligns)
        self.ys_in = copy.deepcopy(ys_in)
        self.ys_out = copy.deepcopy(ys_out)
        reverse_styles = self._get_reverse_style(styles)
        enc_output, enc_lens = self.encoder(xs, ilens)
        _, _, predictions, attns =\
            self.decoder(enc_output, enc_lens, reverse_styles, aligns, None,
                         max_dec_timesteps=self.config['max_dec_timesteps'], sample=True)
        self.prediction = copy.deepcopy(predictions)
        return cycle_loss

    def _log_train(self, epoch, train_steps, total_steps, loss, recon_loss, cycle_loss):
        ...
        self.logger.scalar_summary(tag=f'{tag}/train/recon loss', 
                                   value=recon_loss, 
                                   step=(epoch*total_steps+train_steps+1))
        self.logger.scalar_summary(tag=f'{tag}/train/cycle loss', 
                                   value=cycle_loss, 
                                   step=(epoch*total_steps+train_steps+1))
        return

    def _train_onetime(self, data, total_loss, tf_rate, train_steps, total_steps, epoch):
        cycle_loss = self._get_cycle_loss(data, tf_rate)
        recon_loss = self._get_loss(data, tf_rate, epoch)
        #calcuate gradients
        if epoch <= 7:
            ratio = 0.95-(0.95-0.25)*((epoch-0.0)/(7.0-0.0))
        else:
            ratio = 0.25
        loss = recon_loss*ratio + cycle_loss*(1-ratio)
        self.optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.params, max_norm=self.config['max_grad_norm'])
        self.optimizer.step()
        total_loss += loss.item()
        self._log_train(epoch, train_steps, total_steps, loss, recon_loss, cycle_loss)
        train_steps +=1
