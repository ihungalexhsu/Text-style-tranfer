...
def to_gpu(data, bos, eos, pad):
    input_data, output_data, ilens, labels = data
    ilens = cc(torch.LongTensor(ilens))
    labels = cc(labels)
    ys = [cc(y) for y in output_data]
    ...
    xs = [cc(x) for x in input_data]
    xs = pad_list(xs, pad_value=pad)
...
def getw2v(w2vpath, vocabdict, pad_idx, binary=False):
    model = KeyedVectors.load_word2vec_format(w2vpath, binary=binary)
    modelwordlist = model.index2entity
    pretrainweight = list()
    for i,(k,v) in enumerate(vocabdict.items()):
...
def getembwordlist(w2vpath, binary=False):
    model = KeyedVectors.load_word2vec_format(w2vpath, binary=binary)
    modelwordlist = model.index2entity
