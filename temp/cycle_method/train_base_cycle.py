...
class Base_cycle(object):
    ...
    def _get_loss(self, data, tf_rate, train=False):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        ...
        _, recon_log_probs, predictions, attns = self.decoder(enc_output, enc_lens, styles, (ys_in, ys_out), tf_rate)
        # Reconstruction loss
    ...
    def _train_onetime(self, data, total_loss, tf_rate, train_steps, total_steps, epoch):
        loss = self._get_loss(data, tf_rate, True)
        #calcuate gradients
    ...
    def _valid_reverse_style(self, data_loader):
        ...
        for step, data in enumerate(data_loader):
            ...
            _, _, predictions, attns =\
                self.decoder(enc_output, enc_lens, reverse_styles, None,
