...
import numpy as np
from model_dynamic_withdis import MaskGen, Encoder, Decoder, LSTMAttClassifier, Embedding
...
class Base_dynamic_with_dis(object):
    ...
    def build_model(self, load_model=False):
        ...
        if pretrain_w2v_path is None:
            ...
            pretrain_w2v = pickle.load(open(pretrain_w2v_path,'rb'))
        
        self.maskgen = cc_model(MaskGen(vocab_size=len(self.vocab),
                                        embedding_dim=self.config['embedding_dim'],
                                        hidden_dim=self.config['maskgen_dim'],
                                        pad=self.vocab['<PAD>'],
                                        bidirectional=self.config['bidirectional']))
        print(self.maskgen)
        self.maskgen.float()
        self.embedding = cc_model(Embedding(vocab_size=len(self.vocab),
                                            embedding_dim=self.config['embedding_dim'],
                                            pad=self.vocab['<PAD>'],
        ...
        self.encoder = cc_model(Encoder(embedding_dim=self.config['embedding_dim'],
        ...
        if self.config['bidirectional']: 
            enc_out_dim=self.config['enc_hidden_dim']*2 
        else:
        ...
        self.decoder.float()
        '''
        self.classifier = cc_model(LSTMAttClassifier(embedding_dim=self.config['embedding_dim'],
                                                     rnn_hidden_dim=self.config['cls_dim'],
                                                     dropout_rate=self.config['cls_dropout_p'],
                                                     dnn_hidden_dim=self.config['cls_dim'],
                                                     attention_dim=self.config['cls_att_dim']))
        print(self.classifier)
        self.classifier.float()
        '''
        self.discriminator = cc_model(LSTMAttClassifier(embedding_dim=self.config['embedding_dim'],
                                                        rnn_hidden_dim=self.config['cls_dim'],
                                                        dropout_rate=self.config['cls_dropout_p'],
                                                        dnn_hidden_dim=self.config['cls_dim'],
                                                        attention_dim=self.config['cls_att_dim']))
        ...
        #self.params=list(self.maskgen.parameters())+list(self.encoder.parameters())+\
        #    list(self.decoder.parameters())+list(self.classifier.parameters())
        self.params=list(self.encoder.parameters())+list(self.decoder.parameters())+\
            list(self.embedding.parameters())+list(self.discriminator.parameters())
        self.params_1=list(self.discriminator.parameters())
        self.params_2=list(self.maskgen.parameters())
        self.optimizer=\
            torch.optim.Adam(self.params, 
                             lr=self.config['learning_rate'],
                             weight_decay=float(self.config['weight_decay']))
        self.optimizer_1=\
            torch.optim.Adam(self.params_1, 
                             lr=self.config['learning_rate'],
                             weight_decay=float(self.config['weight_decay']))
        self.optimizer_2=\
            torch.optim.Adam(self.params_2, 
                             lr=self.config['learning_rate'],
                             weight_decay=float(self.config['weight_decay']))
        if load_model:
    ...
    def load_model(self, model_path, load_optimizer):
        ...
        if os.path.exists(model_path+'_enc.ckpt'):
            ...
            self.maskgen.load_state_dict(torch.load(f'{model_path}_maskgen.ckpt'))
            self.encoder.load_state_dict(torch.load(f'{model_path}_enc.ckpt'))
            self.decoder.load_state_dict(torch.load(f'{model_path}_dec.ckpt'))
            #self.classifier.load_state_dict(torch.load(f'{model_path}_cls.ckpt'))
            self.embedding.load_state_dict(torch.load(f'{model_path}_emb.ckpt'))
            self.discriminator.load_state_dict(torch.load(f'{model_path}_dis.ckpt'))
            if load_optimizer:
                print(f'Load optmizer from {model_path}')
                self.optimizer.load_state_dict(torch.load(f'{model_path}.opt'))
                self.optimizer_1.load_state_dict(torch.load(f'{model_path}.opt1'))
                self.optimizer_2.load_state_dict(torch.load(f'{model_path}.opt2'))
                if self.config['adjust_lr']:
                    adjust_learning_rate(self.optimizer, self.config['retrieve_lr']) 
                    adjust_learning_rate(self.optimizer_1, self.config['retrieve_lr']) 
        return
  
    def save_model(self, model_path):
        model_path = model_path+'_b'+str(self.beta)+'g'+str(self.gamma)
        torch.save(self.maskgen.state_dict(), f'{model_path}_maskgen.ckpt')
        torch.save(self.encoder.state_dict(), f'{model_path}_enc.ckpt')
        torch.save(self.decoder.state_dict(), f'{model_path}_dec.ckpt')
        #torch.save(self.classifier.state_dict(), f'{model_path}_cls.ckpt')
        torch.save(self.embedding.state_dict(), f'{model_path}_emb.ckpt')
        torch.save(self.discriminator.state_dict(), f'{model_path}_dis.ckpt')
        torch.save(self.optimizer.state_dict(), f'{model_path}.opt')
        torch.save(self.optimizer_1.state_dict(), f'{model_path}.opt1')
        torch.save(self.optimizer_2.state_dict(), f'{model_path}.opt2')
        return
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            if score > best_score: 
                ...
                best_maskgen = copy.deepcopy(self.maskgen.state_dict())
                best_enc = copy.deepcopy(self.encoder.state_dict())
                best_dec = copy.deepcopy(self.decoder.state_dict())
                best_emb = copy.deepcopy(self.embedding.state_dict())
        ...
        base_model = (best_maskgen, best_enc, best_dec, best_emb)
    ...
    def _get_random_style(self, styles):
        random_styles = styles.cpu().new_zeros(styles.size())
        for idx in range(len(styles.cpu().tolist())):
            if np.random.random() > 0.5:
                random_styles[idx]=1
        random_styles=cc(torch.LongTensor(random_styles))
        return random_styles

    def _get_loss_mask(self, data, tf_rate):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        xs, ys, ys_in, ys_out, ilens, styles, aligns = to_gpu(data, bos, eos, pad)
        mask, _ = self.maskgen(xs, ilens)
        emb = self.embedding(xs)
        style_words = emb.detach()*mask
        content_words = emb.detach() - style_words
        enc_outputs, enc_lens = self.encoder(content_words, ilens)
        _, recon_log_probs, predictions, attns =\
            self.decoder(enc_outputs, enc_lens, styles, (ys_in, ys_out), tf_rate,
                         self.config['max_dec_timesteps'])
        _, dis_log_probs, dis_predicts, _ = self.discriminator(style_words, ilens)
        ...
        # discriminator loss
        '''
        random_styles = self._get_random_style(styles)
        true_dislog_probs = torch.gather(dis_log_probs, dim=1,
                                         index=random_styles.unsqueeze(1)).squeeze(1)
        '''
        true_dislog_probs = torch.gather(dis_log_probs, dim=1,
                                         index=styles.unsqueeze(1)).squeeze(1)
        correct = styles.eq(dis_predicts.view(-1).long()).sum().item()
        acc = float(correct)/float(styles.size(0))
        dis_loss = -torch.mean(true_dislog_probs)*self.gamma
        loss = recon_loss+mask_loss+dis_loss
        return loss, recon_loss, mask_loss, dis_loss, acc 
    
    def _log_train_mask(self, epoch, cnt, total_steps, recon_loss, mask_loss,
                        dis_loss, acc):
        
        tag = self.config['tag']
        self.logger.scalar_summary(tag=f'{tag}/train/mask loss', 
                                   value=mask_loss,
                                   step=(epoch*total_steps+cnt+1))
        self.logger.scalar_summary(tag=f'{tag}/train/mask recon loss', 
                                   value=recon_loss,
                                   step=(epoch*total_steps+cnt+1))
        self.logger.scalar_summary(tag=f'{tag}/train/mask discri loss', 
                                   value=dis_loss.item()/self.gamma,
                                   step=(epoch*total_steps+cnt+1))
        self.logger.scalar_summary(tag=f'{tag}/train/classification acc', 
                                   value=acc,
                                   step=(epoch*total_steps+cnt+1))
        return

    def _get_loss_dis(self, data):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        xs, ys, ys_in, ys_out, ilens, styles, aligns = to_gpu(data, bos, eos, pad)
        mask, _ = self.maskgen(xs, ilens)
        emb = self.embedding(xs)
        style_words = emb*mask
        content_words = emb - style_words
        _, dis_log_probs, dis_predicts, _ = self.discriminator(content_words.detach(), ilens)
        # discriminator loss
        true_dislog_probs = torch.gather(dis_log_probs, dim=1,
                                         index=styles.unsqueeze(1)).squeeze(1)
        dis_loss = -torch.mean(true_dislog_probs)
        return dis_loss 
    
    def _log_train_dis(self, epoch, total_steps, cnt, dis_loss):
        # add to logger
        tag = self.config['tag']
        self.logger.scalar_summary(tag=f'{tag}/train/adv discriminator loss', 
                                   value=dis_loss.item(),
                                   step=(epoch*total_steps+cnt+1))
        return

    def _get_recon_loss(self, data, tf_rate, to_round=False):
        bos = self.vocab['<BOS>']
        eos = self.vocab['<EOS>']
        pad = self.vocab['<PAD>']
        xs, ys, ys_in, ys_out, ilens, styles, aligns = to_gpu(data, bos, eos, pad)
        ...
        '''
        if to_round:
            style_words = emb*torch.round(mask.detach())
        else:
            style_words = emb*mask.detach()
        '''
        style_words = emb*torch.round(mask.detach())
        content_words = emb - style_words
        #_, cls_log_probs, cls_predicts, _ = self.classifier(style_words, ilens)
        enc_outputs, enc_lens = self.encoder(content_words, ilens)
        _, recon_log_probs, predictions, attns =\
            self.decoder(enc_outputs, enc_lens, styles, (ys_in, ys_out), tf_rate,
                         self.config['max_dec_timesteps'])
        ...
        true_dislog_probs = torch.gather(dis_log_probs, dim=1,
                                         index=styles.unsqueeze(1)).squeeze(1)
        dis_loss = -torch.mean(true_dislog_probs)
        loss = recon_loss+cover_loss+dis_loss
        return loss, recon_loss, cover_loss, dis_loss
    
    def _log_train(self, epoch, train_steps, total_steps, loss, 
                   recon_loss, cover_loss):
    ...
    def _train_onetime(self, data, total_loss, tf_rate, train_steps, total_steps, epoch):
        loss, recon_loss, cover_loss, dis_loss = self._get_recon_loss(data, tf_rate)
        #calcuate gradients
        ...
        self._log_train(epoch, train_steps, total_steps, loss, recon_loss, cover_loss)
        self._log_train_dis(epoch, total_steps, train_steps, dis_loss)
        train_steps +=1
        return (train_steps, total_loss)
    
    def _train_pretrain(self, data):
        loss, recon_loss, cover_loss = self._get_recon_loss(data, 1.0)
        #calcuate gradients
        self.optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.params, max_norm=self.config['max_grad_norm'])
    ...
    def _train_dis_onetime(self, data, total_steps, epoch, cnt):
        adv_dis_loss = self._get_loss_dis(data)
        self.optimizer_1.zero_grad()
        adv_dis_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.params_1, max_norm=self.config['max_grad_norm'])
        self.optimizer_1.step()
        self._log_train_dis(epoch, total_steps, cnt, adv_dis_loss)
        cnt += 1
        return cnt

    def _train_mask_onetime(self, data, total_steps, epoch, cnt, tf_rate):
        loss, recon_loss, mask_loss, dis_loss, acc = self._get_loss_mask(data, tf_rate)
        self.optimizer_2.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.params_2, max_norm=self.config['max_grad_norm'])
        self.optimizer_2.step()
        self._log_train_mask(epoch, cnt, total_steps, recon_loss, mask_loss, dis_loss, acc)
        cnt += 1
        return cnt
    
    def pretrain_recon(self):
    ...
    def train_one_epoch(self, epoch, tf_rate):
        ...
        if epoch < 4:
            ...
            cnt = 0
            for _ in range(4):
                pos_data_iterator_1 = iter(self.train_pos_loader)
                neg_data_iterator_1 = iter(self.train_neg_loader)
                for _ in range(len(self.train_pos_loader)):
                    try:
                        data = next(pos_data_iterator_1)
                    except StopIteration:
                        print('StopIteration in pos part')
                        pass
                    cnt = self._train_dis_onetime(data, total_steps, epoch, cnt)
                    try:
                        data = next(neg_data_iterator_1)
                    except StopIteration:
                        neg_data_iterator_1 = iter(self.train_neg_loader)
                        data = next(neg_data_iterator_1)
                    cnt = self._train_dis_onetime(data, total_steps, epoch, cnt)
            ...
            cnt = 0
            for _ in range(1):
                pos_data_iterator_2 = iter(self.train_pos_loader)
                neg_data_iterator_2 = iter(self.train_neg_loader)
                for _ in range(len(self.train_pos_loader)):
                    try:
                        data = next(pos_data_iterator_2)
                    except StopIteration:
                        print('StopIteration in pos part')
                        pass
                    cnt = self._train_mask_onetime(data, total_steps, epoch, cnt, tf_rate)
                    try:
                        data = next(neg_data_iterator_2)
                    except StopIteration:
                        neg_data_iterator_2 = iter(self.train_neg_loader)
                        data = next(neg_data_iterator_2)
                    cnt = self._train_mask_onetime(data, total_steps, epoch, cnt, tf_rate)
    ...
    def _valid_reverse_style(self, data_loader):
        ...
        for step, data in enumerate(data_loader):
            ...
            # create masked_list
            masks = torch.round(mask).squeeze(2)
            masked_sents = list()
            for i,y in enumerate(ys):
                sent = list()
                for j,w in enumerate(y):
                    if masks[i][j]:
                        sent.append(self.vocab['<PAD>'])
                    else:
                        sent.append(w.cpu().item())
                masked_sents.append(sent)
            all_masked = all_masked + masked_sents
        return all_prediction, all_inputs, all_masked
    ...
    def _log_valid(self, epoch, steps, total_steps, loss,
        ...
        # add to logger
        tag = self.config['tag']
        self.logger.scalar_summary(tag=f'{tag}/val/val loss', 
                                   value=loss, 
    ...
    def _valid_recons(self, epoch, data_loader, total_loss, step_start, total_steps):
        for step, data in enumerate(data_loader):
            loss, recon_loss, cover_loss, _ = self._get_recon_loss(data, 0.)
            total_loss += loss.item()
    ...
    def test(self, state_dict=None):
        ...
        if not state_dict:
            ...
            self.maskgen.load_state_dict(state_dict[0])
            self.encoder.load_state_dict(state_dict[1])
            self.decoder.load_state_dict(state_dict[2])
            self.embedding.load_state_dict(state_dict[3])
        ...
        self.discriminator.eval()
        # positive input
        posdata_pred, posdata_input, posdata_inputmask =\
            self._valid_reverse_style(test_pos_loader)
        # get sentence
        posdata_pred, posdata_input = self.idx2sent(posdata_pred, posdata_input)
        _, posdata_inputmask = self.idx2sent(posdata_inputmask, copy.deepcopy(posdata_inputmask))
        # write file
        ...
        file_path_gtpos = os.path.join(write_file_path, 
                                       f'base.test.1.input')
        file_path_posmask = os.path.join(write_file_path, 
                                     f'base.test.1.maskinput')
        writefile(posdata_pred, file_path_pos)
        writefile(posdata_input, file_path_gtpos)
        writefile(posdata_inputmask, file_path_posmask)
        # negative input
        negdata_pred, negdata_input, negdata_inputmask =\
            self._valid_reverse_style(test_neg_loader)
        # get sentence
        negdata_pred, negdata_input = self.idx2sent(negdata_pred, negdata_input)
        _, negdata_inputmask = self.idx2sent(negdata_inputmask, copy.deepcopy(negdata_inputmask))
        # write file
        file_path_neg = os.path.join(write_file_path, 
                                     f'base.test.0to1.input')
        file_path_gtneg = os.path.join(write_file_path, 
                                       f'base.test.0.input')
        file_path_negmask = os.path.join(write_file_path, 
                                     f'base.test.0.maskinput')
        writefile(negdata_pred, file_path_neg)
        writefile(negdata_input, file_path_gtneg)       
        writefile(negdata_inputmask, file_path_negmask)
