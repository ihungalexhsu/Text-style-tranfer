...
class Pretrain_selfatt_classifier(object):
    ...
    def get_important_word(self, input_words, att_energy, emo_word_list=None):
        ...
        emotion_alignment = self.tune_alignment(input_words, emotion_alignment, emo_word_list)
    ...
    def tune_alignment(self, input_words, emotion_alignment, emo_word_list=None):
        ...
        if emo_word_list is None:
            ...
            emo_word_id = list()
            for w in emo_word_list:
                if w in self.vocab.keys():
                    emo_word_id.append(self.vocab[w])
            for i,(ws, masks) in enumerate(zip(input_words, emotion_alignment)):
                for j,(w, m) in enumerate(zip(ws, masks)):
                    if (m) and (w.cpu().int().item() not in self.stop_words_ids) and \
                            (w.cpu().int().item() in emo_word_id):
                        new_alignment[i,j]=1
                    else:
                        new_alignment[i,j]=0
    ...
    def collect_alignment(self, data_loader, emo_word_collect=None):
        self.classifier.eval()
        if emo_word_collect is None:
            ...
            print("emo_word_counter size", len(emo_word_collect))
            emo_word_collect = emo_word_collect.most_common(1000)
            emo_word_collect = [w for w,k in emo_word_collect]
        ...
        for data in data_loader:
            ...
            xs, ys, ys_in, ys_out, ilens, styles = to_gpu(data, bos, eos, pad)
            logits, log_probs, prediction, att_energy = self.classifier(xs, ilens)
            emo_words, emotion_alignment = self.get_important_word(xs, att_energy, emo_word_collect)
            ...
            for i in range(len(ori_data)):
                temp_data={
                    'data':ori_data[i],
                    'label':ori_style[i],
                    'align':torch.ByteTensor(emotion_alignment[i].cpu()),
                    'words':ys[i]
                }
    ...
    def get_alignmentoutput(self):
        ...
        pos_data_path = os.path.join(store_root, 'pos_train_selfattmask.p')
        ...
        neg_data_path = os.path.join(store_root, 'neg_train_selfattmask.p')
        pickle.dump(list_todictformat(train_neg_data), open(neg_data_path,'wb'))
        del train_neg_data
        
        idx2vocab = [w for w in self.vocab.keys()]
        dev_pos_data, _ = self.collect_alignment(self.dev_pos_loader, train_pos_emo)
        pos_data_path = os.path.join(store_root, 'pos_dev_selfattmask.p')
        pickle.dump(list_todictformat(dev_pos_data), open(pos_data_path,'wb'))
        with open(os.path.join(store_root, 'pos_dev_with_SelfAttMaskOut.txt'), 'w') as f:
            for d in dev_pos_data:
                data = d['data']
                align = d['align']
                for i, word in enumerate(data):
                    if align[i].item():
                        ...
                        f.write(idx2vocab[word])
                    f.write(' ')
                f.write('\n')
        del dev_pos_data
        
        dev_neg_data, _ = self.collect_alignment(self.dev_neg_loader, train_neg_emo)
        neg_data_path = os.path.join(store_root, 'neg_dev_selfattmask.p')
        pickle.dump(list_todictformat(dev_neg_data), open(neg_data_path,'wb'))
        with open(os.path.join(store_root, 'neg_dev_with_SelfAttMaskOut.txt'), 'w') as f:
            for d in dev_neg_data:
                data = d['data']
                align = d['align']
                for i, word in enumerate(data):
                    if align[i].item():
        ...
        test_neg_loader = get_data_loader(test_neg_dataset, 
                                          batch_size=16, 
                                          shuffle=False)
        test_pos_data, _ = self.collect_alignment(test_pos_loader, train_pos_emo)
        pos_data_path = os.path.join(store_root, 'reference.1.selfattmask.p')
        pickle.dump(list_todictformat(test_pos_data), open(pos_data_path,'wb'))
        with open(os.path.join(store_root, 'reference.1.SelfAttMaskOut.txt'), 'w') as f:
            for d in test_pos_data:
                data = d['data']
                align = d['align']
                for i, word in enumerate(data):
                    if align[i].item():
        ...
        test_neg_data, _ = self.collect_alignment(test_neg_loader, train_neg_emo)
        neg_data_path = os.path.join(store_root, 'reference.0.selfattmask.p')
        pickle.dump(list_todictformat(test_neg_data), open(neg_data_path,'wb'))
        with open(os.path.join(store_root, 'reference.0.SelfAttMaskOut.txt'), 'w') as f:
            for d in test_neg_data:
                data = d['data']
                align = d['align']
                for i, word in enumerate(data):
                    if align[i].item():
