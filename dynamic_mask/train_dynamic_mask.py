...
import numpy as np
from dynamic_mask.model import E2E
from dynamic_mask.dataloader import get_data_loader
from dynamic_mask.dataset import PickleDataset
from dynamic_mask.utils import *
...
class Dynamic_mask(object):
    def __init__(self, config, beta=10, gamma=10, load_model=False):
        self.config = config
        print(self.config)
        self.beta = float(beta) # ratio of make rationale short
        self.gamma = float(gamma) # ratio of reconstruction
        # logger
    ...
    def build_model(self, load_model=False):
        ...
        self.model = cc_model(E2E(vocab_size=len(self.vocab),
                                  ...
                                  detach_recons=self.config['detach_recons'],
    ...
    def _get_loss(self, data, training=True):
        ...
        if training:
            mask, cls_log_probs, cls_predicts, recon_log_probs, predictions =\
        ...
        '''
        L1loss = nn.L1Loss()
        target = mask.new_zeros(mask.size())
        mask_loss = L1loss(mask, target)*self.beta
        '''
        mask_loss = 0.
        # Reconstruction loss
        recon_loss = -torch.mean(recon_log_probs)*self.gamma
        # Classifier loss
        true_log_probs = torch.gather(cls_log_probs, dim=1,
                                      index=styles.unsqueeze(1)).squeeze(1)
        cls_loss = -torch.mean(true_log_probs)*1
        correct = styles.eq(cls_predicts.view(-1).long()).sum().item()
        acc = float(correct)/float(styles.size(0))
        loss = mask_loss+recon_loss+cls_loss
        return loss, mask_loss, recon_loss, cls_loss, acc
    
    def _log_train(self, epoch, train_steps, total_steps, loss, mask_loss,
                   recon_loss, cls_loss, acc):
    ...
    def _train_onetime(self, data, total_loss, train_steps, total_steps, epoch):
        loss, mask_loss, recon_loss, cls_loss, acc = self._get_loss(data)
        #calcuate gradients
        ...
        self._log_train(epoch, train_steps, total_steps, loss, mask_loss, recon_loss, cls_loss, acc)
        return total_loss
    ...
    def _log_valid(self, epoch, steps, total_steps, loss,
                   mask_loss, recon_loss, cls_loss, acc):
    ...
    def _valid_recons(self, epoch, data_loader, total_loss, step_start, total_steps):
        for step, data in enumerate(data_loader):
            loss, mask_loss, recon_loss, cls_loss, acc = self._get_loss(data, False)
            total_loss += loss.item()
