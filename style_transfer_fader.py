...
import numpy as np
from model import Encoder, Decoder, Style_classifier, TextCNN, Domain_discri
...
class Style_transfer_fader(object):
    ...
    def load_automatic_style_classifier(self, model_path, train=False):
        ...
        '''
        self.textcnn = cc_model(TextCNN(vocab_size=len(self.vocab),
                                        kernel_num=200,
                                        kernel_size=[3,4,5],
                                        embedded_size=100,
                                        dropout_p=0.2,
                                        dnn_hidden=300,
                                        output_class=self.config['n_style_type'],
                                        embedding=pretrain_w2v,
        ...
        if (model_path is None) or (train):
            ...
            #self.textcnn.load_state_dict(best_model)
            self.textrnn.load_state_dict(best_model)
        else:
            #self.textcnn.load_state_dict(torch.load(f'{model_path}.ckpt'))
    ...
    def train_automatic_style_classifier(self, load=False, model_path=None): 
        ...
        best_model = None
        #opt_pretrain_sc = torch.optim.Adam(list(self.textcnn.parameters()), 
        ...
        for epoch in range(20):
            ...
            for train_steps, data in enumerate(self.train_lab_loader):
                ...
                #log_probs, prediction = self.textcnn(xs)
            ...
            #torch.save(self.textcnn.state_dict(), f'{model_path}-{epoch:03d}.ckpt')
            torch.save(self.textrnn.state_dict(), f'{model_path}-{epoch:03d}.ckpt')
            #self.textcnn.train()
            self.textrnn.train()
            if val_acc > best_acc: 
                # save model
                #torch.save(self.textcnn.state_dict(), f'{model_path}_best.ckpt')
                torch.save(self.textrnn.state_dict(), f'{model_path}_best.ckpt')
                best_acc = val_acc
                #best_model = self.textcnn.state_dict()
    ...
    def train_one_epoch(self, epoch, tf_rate):
        ...
        for cnt in range(self.config['m2_train_freq']):
            for train_steps, data in enumerate(self.train_lab_loader):
                ...
                s_loss= -torch.mean(true_s_log_probs)*self.config['style_loss_ratio']            
                total_inverse_style_loss += s_loss.item()           
                ...
                print(f'epoch: {epoch}, [{train_steps + 1}/{total_steps}],'
                      f'style_loss: {s_loss:.3f},', end='\r')
                # add to logger
                tag = self.config['tag']
                self.logger.scalar_summary(tag=f'{tag}/train/style_classifier_loss', 
                                           value=s_loss.item()/self.config['style_loss_ratio'], 
        ...
        for train_steps, data in enumerate(self.train_lab_loader):
            ...
            loss = -torch.mean(recon_log_probs)*self.config['recons_loss_ratio']
            total_loss += loss.item()
            # Style Loss
            enc_representation = get_enc_context(enc_outputs, enc_lens)
            _, s_log_probs, _= self.s_classifier(enc_representation)
            ...
            s_loss = -torch.mean(random_s_log_probs)*self.config['style_loss_ratio']            
            ...
            self.logger.scalar_summary(tag=f'{tag}/train/reconloss', 
                                       value=loss.item()/self.config['recons_loss_ratio'], 
                                       step=epoch*total_steps+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/adver_styleloss', 
                                       value=s_loss.item()/self.config['style_loss_ratio'], 
                                       step=epoch*total_steps+train_steps+1)
        print()
        return ((total_loss/total_steps),(total_style_loss/total_steps),\
                (total_inverse_style_loss/total_steps)/self.config['m2_train_freq'])
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            # train one epoch
            avgl_re, avgl_s, avgl_invs = self.train_one_epoch(epoch, tf_rate)
            # validation
            avg_valid_loss, wer, prediction_sents, ground_truth_sents, avg_style_acc = self.validation()

            print(f'Epoch: {epoch}, tf_rate={tf_rate:.3f}, Recon_l={avgl_re:.4f},'
                  f'Adver_style_l:{avgl_s:.4f}, Style_classify_l:{avgl_invs:.4f},'
