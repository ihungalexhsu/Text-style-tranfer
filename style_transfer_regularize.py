...
class Style_transfer_regularize(object):
    def __init__(self, config, alpha=10, beta=1, gamma=10, delta=100, zeta=100, eta=100, load_model=False):
        ...
        self.logger = Logger(config['logdir']+'_a'+str(alpha)+'b'+str(beta)+'g'+\
                             str(gamma)+'d'+str(delta)+'z'+str(zeta)+'e'+str(eta))
    ...
    def save_model(self, model_path):
        model_path = (model_path+'_a'+str(self.alpha)+'b'+str(self.beta)+'g'+\
            str(self.gamma)+'d'+str(self.delta)+'z'+str(self.zeta)+'e'+str(self.eta))
    ...
    def build_model(self, load_model=False):
        ...
        '''
        self.disenC = cc_model(Style_classifier(enc_out_dim=encC_out_dim,
                                                hidden_dim=self.config['s_classifier_hidden_dim'],
                                                n_layers=self.config['s_classifier_n_layers'],
                                                out_dim=self.config['n_style_type']))
        print(self.disenC)
    ...
    def validation(self):
        ...
        for step, data in enumerate(self.dev_loader):
            ...
            #mimicked_style_vector = self.style_mimicker(reverse_styles.float().view(-1,1))
    ...
    def test(self, state_dict=None):
        ...
        for step, data in enumerate(test_loader):
            ...
            #mimicked_style_vector = self.style_mimicker(half_styles.float().view(-1,1))
            decoder_input = torch.cat([mimicked_style_vector, content_vector], dim=1)
            _, recon_log_probs, prediction, _=\
                self.decoder(decoder_input, enc_lens, reverse_styles, None,
                             max_dec_timesteps=self.config['max_dec_timesteps'])
            ...
            #mimicked_style_vector = self.style_mimicker(styles.float().view(-1,1))
            decoder_input = torch.cat([mimicked_style_vector, content_vector], dim=1)
            _, recon_log_probs, prediction, _=\
                self.decoder(decoder_input, enc_lens, reverse_styles, None,
                             max_dec_timesteps=self.config['max_dec_timesteps'])
    ...
    def train_one_epoch(self, epoch, tf_rate, mean_sv_pos, mean_sv_neg):
        ...
        for train_steps, data in enumerate(zip(self.train_pos_loader,self.train_neg_loader)):
            ...
            s_loss_pos = -torch.mean(true_s_log_probs)*self.beta
            total_sloss += s_loss_pos.item()
            # Disentangle Loss
            enc_outputs, enc_lens = self.encC(xs, ilens)
            content_vector = get_enc_context(enc_outputs, enc_lens)
            predict_content = self.disenS(style_vector)
            rand_vec2 = self._random_vector(content_vector, 'tanh')
            loss_adv_disencS_pos = torch.mean((predict_content-rand_vec2)**2)*self.delta
            ...
            mimicked_style_vector = self.style_mimicker(torch.cat([styles.float().view(-1,1),content_vector],dim=1))
            #mimicked_style_vector = self.style_mimicker(styles.float().view(-1,1))
            loss_mimic_pos = torch.mean((mimicked_style_vector-style_vector)**2)*self.gamma
            total_mimic += loss_mimic_pos.item()
            # Reconstruction loss
            decoder_input = torch.cat([mimicked_style_vector, content_vector], dim=1)
            ...
            loss_recon_pos = -torch.mean(recon_log_probs)*self.alpha
            total_recon += loss_recon_pos.item()
            # Regularizer on style embedding
            loss_cos_pos = torch.mean(CosSim(style_vector, mean_style_vector_neg.expand_as(style_vector))+1)*self.eta
            total_cos_loss += loss_cos_pos.item()
            mean_style_vector_pos = torch.mean(style_vector, dim=0)
            loss_cluster_pos = torch.mean((style_vector-mean_style_vector_pos.expand_as(style_vector))**2)*(self.eta/5)
            total_cluster_loss += loss_cluster_pos.item()
            ...
            true_s_log_probs = torch.gather(s_log_probs,dim=1,
                                            index=styles.unsqueeze(1)).squeeze(1)
            s_loss_neg = -torch.mean(true_s_log_probs)*self.beta
            total_sloss += s_loss_neg.item()
            # Disentangle Loss
            predict_content = self.disenS(style_vector)
            enc_outputs, enc_lens = self.encC(xs, ilens)
            content_vector = get_enc_context(enc_outputs, enc_lens)
            rand_vec2 = self._random_vector(content_vector, 'tanh')
            loss_adv_disencS_neg = torch.mean((predict_content-rand_vec2)**2)*self.delta
            #loss_adv_disencS = -torch.mean((predict_content-content_vector)**2)*self.delta
            '''
            predict_style = self.disenC(content_vector)
            rand_vec1 = self._random_vector(style_vector, 'tanh')
            ...
            mimicked_style_vector = self.style_mimicker(torch.cat([styles.float().view(-1,1),content_vector],dim=1))
            #mimicked_style_vector = self.style_mimicker(styles.float().view(-1,1))
            loss_mimic_neg = torch.mean((mimicked_style_vector-style_vector)**2)*self.gamma
            total_mimic += loss_mimic_neg.item()
            # Reconstruction loss
            decoder_input = torch.cat([mimicked_style_vector, content_vector], dim=1)
            _, recon_log_probs, _, _=\
                self.decoder(decoder_input, enc_lens, styles, (ys_in, ys_out),
            ...
            loss_recon_neg = -torch.mean(recon_log_probs)*self.alpha
            total_recon += loss_recon_neg.item()
            # Regularizer on style embedding
            loss_cos_neg = torch.mean(CosSim(style_vector, mean_style_vector_pos.expand_as(style_vector))+1)*self.eta
            total_cos_loss += loss_cos_neg.item()   
            mean_style_vector_neg = torch.mean(style_vector, dim=0)
            loss_cluster_neg = torch.mean((style_vector-mean_style_vector_neg.expand_as(style_vector))**2)*(self.eta/5)
            total_cluster_loss += loss_cluster_neg.item()
            
            # calculate gradients 
            self.optimizer_m1.zero_grad()
            tloss = s_loss_pos+loss_adv_disencS_pos+loss_adv_disencC_pos+loss_mimic_pos+loss_recon_pos+loss_cos_pos+loss_cluster_pos
            tloss += s_loss_neg+loss_adv_disencS_neg+loss_adv_disencC_neg+loss_mimic_neg+loss_recon_neg+loss_cos_neg+loss_cluster_neg
            ...
            self.logger.scalar_summary(tag=f'{tag}/train/reconloss', 
                                       value=(loss_recon_pos.item()+loss_recon_neg.item())/(2*self.alpha), 
                                       step=epoch*total_steps/2+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/styleloss', 
                                       value=(s_loss_pos.item()+s_loss_neg.item())/(2*self.beta), 
                                       step=epoch*total_steps/2+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/adver_disencS_loss', 
                                       value=(loss_adv_disencS_pos.item()+loss_adv_disencS_neg.item())/(2*self.delta), 
                                       step=epoch*total_steps/2+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/adver_disencC_loss', 
                                       value=(loss_adv_disencC_pos.item()+loss_adv_disencC_neg.item())/(2*self.zeta), 
                                       step=epoch*total_steps/2+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/mimic_loss', 
                                       value=(loss_mimic_pos.item()+loss_mimic_neg.item())/(2*self.gamma), 
                                       step=epoch*total_steps/2+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/style_embedding_cosine_loss', 
                                       value=(loss_cos_pos.item()+loss_cos_neg.item())/(2*self.eta), 
                                       step=epoch*total_steps/2+train_steps+1)
            self.logger.scalar_summary(tag=f'{tag}/train/style_embedding_cluster_loss', 
                                       value=(loss_cluster_pos.item()+loss_cluster_neg.item())/(2*self.eta), 
                                       step=epoch*total_steps/2+train_steps+1)
        print()
        for cnt in range(self.config['m2_train_freq']):
            for train_steps, data in enumerate(zip(self.train_pos_loader,self.train_neg_loader)):
                ...
                loss_disencS_pos = torch.mean((predict_content-content_vector)**2)
                total_disencS += loss_disencS_pos.item()
                '''
                predict_style = self.disenC(content_vector)
                ...
                predict_content = self.disenS(style_vector)
                #loss_disencS = torch.mean((predict_content-content_vector)**2)*self.delta
                loss_disencS_neg = torch.mean((predict_content-content_vector)**2)
                total_disencS += loss_disencS_neg.item()
                '''
                predict_style = self.disenC(content_vector)
                #loss_disencC = torch.mean((predict_style-style_vector)**2)*self.zeta
                ...
                (loss_disencS_pos+loss_disencS_neg+loss_disencC_pos+loss_disencC_neg).backward()
                self.optimizer_m2.step()
                # print message
                print(f'epoch: {epoch}, [{train_steps + 1}/{total_steps/2}], '
                      f'disencS_loss: {(loss_disencS_pos.item()+loss_disencS_neg.item()):.3f}, '
                      f'disencC_loss: {(loss_disencC_pos.item()+loss_disencC_neg.item()):.3f}', end='\r')
                # add to logger
                tag = self.config['tag']
                self.logger.scalar_summary(tag=f'{tag}/train/disentangle_encS_loss', 
                                           #value=loss_disencS.item()/self.delta, 
                                           value=(loss_disencS_pos.item()+loss_disencS_neg.item())/2, 
                                           step=(epoch*(self.config['m2_train_freq'])+cnt)*total_steps/2+train_steps+1)
                self.logger.scalar_summary(tag=f'{tag}/train/disentangle_encC_loss', 
                                           #value=loss_disencC.item()/self.zeta, 
                                           value=(loss_disencC_pos.item()+loss_disencC_neg.item())/2, 
                                           step=(epoch*(self.config['m2_train_freq'])+cnt)*total_steps/2+train_steps+1)
            print ()

        return ((total_recon/total_steps),(total_sloss/total_steps),\
                (total_adv_disencS/total_steps),(total_adv_disencC/total_steps),\
                (total_mimic/total_steps),(total_cos_loss/total_steps),\
                ((total_disencS/total_steps)/self.config['m2_train_freq']),\
                ((total_disencC/total_steps)/self.config['m2_train_freq']),\
                mean_style_vector_pos, mean_style_vector_neg)
    ...
    def train(self):
        ...
        # init mean style vector 
        mean_sv_pos = cc(torch.ones(self.config['encS_hidden_dim']*2))
        mean_sv_neg = cc(torch.ones(self.config['encS_hidden_dim']*2).fill_(-1.))
